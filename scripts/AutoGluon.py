...
_DEFAULT_LOG_FILENAME = '_output_AutoGluon.log'
_LOGGER_LEVEL = 'INFO'
...
def save_figures(df_orig: pd.DataFrame, y: pd.Series,
    ...
    for (_, ii_test), n_fold in zip(cv_outer.split(df, y), range(cv_outer.get_n_splits(df, y))):
        ...
        proba = predictor.predict_proba(X_test).iloc[:, 1]
...
def train_autogluon_presets(X: pd.DataFrame, y: pd.Series, cv_outer, base_path,
                            presets: list[str] = ['medium_quality', 'good_quality', 'high_quality', 'best_quality']):

    def calculate_metrics(model, X, y) -> dict:
        # Predictions
        y_hat_proba = model.predict_proba(X, as_pandas=False)[:, 1]
    ...
    for preset in presets:
        ...
        for (ii_train, ii_test), n_fold in zip(cv_outer.split(X, y), range(cv_outer.get_n_splits(X, y))):
            ...
            _X_train = pd.concat([X_train, y_train], axis=1)
            
            # Measure Training Time by Fold
            start_time_fold = time.time()
            path_model = os.path.join(path_preset, 'Model_' + str(n_fold))
            predictor = TabularPredictor(label=y.name, eval_metric='roc_auc', path=path_model).fit(_X_train, presets=preset)
...
def compute_average_metrics(base_path):
    ...
    presets = [preset for preset in presets if preset.endswith('_quality')]
...
base_path = os.path.join(path_input, 'Autogluon_Models')
...
try:
    train_autogluon_presets(X, y, cv_outer, base_path)
    compute_average_metrics(base_path)

    for preset in ['medium_quality', 'good_quality', 'high_quality', 'best_quality']:
        try:
