...
_DEFAULT_LOG_FILENAME = '_output_TPOT.log'
_LOGGER_LEVEL = 'INFO'
...
def save_figures(df_orig: pd.DataFrame, y: pd.Series, base_path) -> pd.DataFrame:
    ...
    for (_, ii_test), n_fold in zip(cv_outer.split(df, y), range(cv_outer.get_n_splits(df, y))):
        ...
        predictor = joblib.load(os.path.join(path_model, 'final_model.joblib'))
        imputer = joblib.load(os.path.join(path_model, 'final_imputer.joblib'))
...
def train_tpot(X: pd.DataFrame, y: pd.Series, cv_outer, base_path):
    ...
    for (ii_train, ii_test), n_fold in zip(cv_outer.split(X, y), range(cv_outer.get_n_splits(X, y))):
        ...
        if 'PatientID' in X_train.columns:
            _, _ = X_train.pop('PatientID'), X_test.pop('PatientID')
        
        # Measure Training Time by Fold
        start_time_fold = time.time()

        tpot = TPOTClassifier(generations=5, population_size=50, verbosity=0, random_state=SEED) # MOFICARRRRR
        tpot.fit(X_train, y_train)

        # Save Trained Predictor
        path_model = os.path.join(path_preset, 'Model_' + str(n_fold))
        ensure_directory_exists(path_model)
        joblib.dump(tpot.fitted_pipeline_, os.path.join(path_model, 'final_model.joblib'))
        joblib.dump(tpot._fitted_imputer, os.path.join(path_model, 'final_imputer.joblib'))
        
        # Load Predictor
        # predictor = joblib.load(os.path.join(path_model, "final_model"))
        
        metrics_train.append(calculate_metrics(tpot, X_train, y_train))
        metrics_test.append(calculate_metrics(tpot, X_test, y_test))
...
base_path = os.path.join(path_input, 'TPOT_Models')
...
if 'PatientID' in X_cat.columns:
    X_cat.pop('PatientID')

if len(X_cat.columns) != 0:
    logger.info(f'Categorical Features: {X_cat.columns.to_list()}')
    X.drop(columns=X_cat.columns, inplace=True)

    X_cat = pd.get_dummies(X_cat, drop_first=True)
    X = pd.concat([X_cat, X], axis=1)

try:
    train_tpot(X, y, cv_outer, base_path)
    compute_average_metrics(base_path)

    try:
        save_figures(X, y, base_path)
