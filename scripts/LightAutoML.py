...
_DEFAULT_LOG_FILENAME = '_output_LightAutoML.log'
_LOGGER_LEVEL = 'INFO'
...
def train_lightAutoML(X: pd.DataFrame, y: pd.Series, cv_outer, base_path):

    def calculate_metrics(model, X, y) -> dict:
        # Predictions
        y_hat_proba = model.predict(X)
        y_hat_proba = y_hat_proba.data[:, 0]
        
        # Apply Defaoult Threshold
    ...
    for (ii_train, ii_test), n_fold in zip(cv_outer.split(X, y), range(cv_outer.get_n_splits(X, y))):
        ...
        roles = {'target': y_train.name}
        RD = ReportDeco(output_path=path_model) # MODIFICAR

        predictor = RD(TabularAutoML(task=task, reader_params={'random_state': SEED}))
        _ = predictor.fit_predict(df_train, roles=roles, verbose=0)

        # Save Trained Predictor
        ensure_directory_exists(path_model)
        joblib.dump(predictor, os.path.join(path_model, "final_model"))
        
        # Load Predictor
        # predictor = joblib.load(os.path.join(path_model, "final_model"))
        
        metrics_train.append(calculate_metrics(predictor, df_train, y_train))
        metrics_test.append(calculate_metrics(predictor, df_test, y_test))
...
base_path = os.path.join(path_input, 'LightAutoML_Models')
...
try:
    train_lightAutoML(X, y, cv_outer, base_path)
