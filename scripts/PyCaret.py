...
_DEFAULT_LOG_FILENAME = '_output_PyCaret.log'
_LOGGER_LEVEL = 'INFO'
...
def save_figures(df_orig: pd.DataFrame, y: pd.Series, base_path) -> pd.DataFrame:
    ...
    for (_, ii_test), n_fold in zip(cv_outer.split(df, y), range(cv_outer.get_n_splits(df, y))):
        ...
        predictor = study.load_model(os.path.join(path_model,  "final_model"))

        try:
            proba = predictor.predict_proba(X_test)[:, 1]
        except:
            proba = predictor.decision_function(X_test)
...
def train_pyCaret(X: pd.DataFrame, y: pd.Series, cv_outer, base_path):
    ...
    def calculate_metrics(model, X, y) -> dict:
        ...
        try:
            ...
            y_hat_proba = model.decision_function(X)
    ...
    for (ii_train, ii_test), n_fold in zip(cv_outer.split(X, y), range(cv_outer.get_n_splits(X, y))):
        ...
        study = ClassificationExperiment()
        study.setup(df, target = y.name, session_id = SEED)
        predictor = study.compare_models()

        # Save Trained Predictor
        path_model = os.path.join(path_preset, 'Model_' + str(n_fold))
        ensure_directory_exists(path_model)
        study.save_model(predictor, os.path.join(path_model, "final_model"))
...
base_path = os.path.join(path_input, 'PyCaret_Models')
...
try:
    train_pyCaret(X, y, cv_outer, base_path)
